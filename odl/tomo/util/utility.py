...
def euler_matrix(phi, theta=None, psi=None):
    ...
    if theta is None and psi is None:
        ...
        ndim = 2
        phi = np.array(phi, dtype=float, copy=None, ndmin=1)
        theta = psi = 0.0
    else:
        ...
        if psi is None:
            psi = 0.0
        theta = np.array(theta, dtype=float, copy=None, ndmin=1)
        psi = np.array(psi, dtype=float, copy=None, ndmin=1)
        ndim = 3
...
def perpendicular_vector(vec):
    ...
    vec = np.array(vec, dtype=float, copy=None, ndmin=2)
